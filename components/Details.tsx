...
const Details = () => {
  ...
  return (
    <div className='flex w-full flex-wrap justify-center mt-5 gap-8'>
        <div>
            <p className='tracking-wide text-xl font-bold border-b-[1px] pb-3 border-b-gray-200'>Testimonial</p>
            <div className='rounded-xl w-80 h-[375px] border-[1px] mt-8 flex justify-center items-center flex-col gap-4'>
                ...
                <p className='w-[175px] tracking-wide text-silver'>{testimonial.description} </p>
            </div>
        </div>
        <div className='w-[640px] h-[450px] relative rounded-xl justify-center items-center'>
            ...
            <a href={currentEvent.eventLink} className='w-[50%] h-[60%] left-0 right-0 top-0 bottom-0 m-auto absolute bg-transparent rounded-md flex flex-col items-center justify-center gap-2'>
                <p className={`text-white bg-black p-2 rounded-md font-bold text-center tracking-wider ${currentEvent.isDiscount ? 'block' : 'hidden'}`}>{currentEvent.discount}% DISCOUNT</p>
                <p className='text-[32px] leading-8 font-bold text-gray-700'>{currentEvent.titleFirst}</p>
                <p className='text-[32px] leading-10 font-bold text-gray-700'>{currentEvent.titleLast}</p>
                <p className='text-xl text-silver'>Starting @ ${currentEvent.starting}</p>
                <button className='font-bold text-xl text-silver'>SHOP NOW</button>
            </a>
        </div>
        <div>
            <p className=' text-xl font-bold border-b-[1px] pb-3 border-b-gray-200 tracking-wide'>Our Services</p>
            <div className='rounded-xl w-80 h-[375px] border-[1px] p-8 mt-8 flex justify-center flex-col gap-4'>
                {featuresSec.map((each,index)=>
                    ...
                    <div className='w-[50px] h-[50px] flex items-center'><i className={`${each.icon} ${hover==index ? 'text-black' : 'text-salmon'}`}></i></div>
                    <div>
                        <p className='font-bold tracking-wide text-silver'>{each.title}</p>
